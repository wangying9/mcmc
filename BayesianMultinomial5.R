...
if (simulation) {
  ...
  for (k in 2 : xCols) {
    x[,k] = as.integer(runif(nRecord, 0, 9))
  }
  y = rep(0, nRecord) 
  z = rep(0, yLevel)
  p = rep(0, yLevel)
...
betaEstPo = array(0, dim=c(yLevel, xCols)) 
burnin = 500
for (j in 1 : yLevel) {
  for (k in 1 : xCols) {
...
if (simulation) {
  ...
  acc = predict(betaEstLk, yTest, xTest)
  accuracy = sprintf("MLK accuracy is %f;", acc)
  print(accuracy)
  acc = predict(betaEstPo, yTest, xTest)
  accuracy = sprintf("MAP accuracy is %f;", acc)
  print(accuracy)
}
